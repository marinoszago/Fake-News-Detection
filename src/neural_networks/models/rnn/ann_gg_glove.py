...
dataset = pd.read_csv('F:\\zagkotsis\\data\\embeddings\\glove_data\\glove_data_ready\\gloveAllTypes.csv',header=None)
dataset = dataset.dropna()
# print(dataset.head())
# Create your connection.
# cnx = sqlite3.connect('F:\\zagkotsis\\data\\embeddings\\glove_data\\glove_data_ready\\gloveAllTypes.db')

# df = pd.read_sql("select * from glove_data LIMIT 10", cnx)
#
# chunk_counter = 1
# for chunk in pd.read_sql_query("select * from glove_data", cnx, chunksize=100000,index_col='index'):
#         print("I am in chunk {}".format(chunk_counter))
#         # chunk.drop(columns=['index'],axis=1, inplace=True)
#         chunk.to_csv('F:\\zagkotsis\\data\\embeddings\\glove_data\\glove_data_ready\\gloveAllTypes.csv',mode='a',header=False,index=False)
#         chunk_counter += 1
#
...
def build_ann(activation1, activation2, activation3, activation4, dropout, optimizer):
    ...
    classifier.add(Dense(units=128, activation=activation1, input_dim=300))
